...
export default function SeatingSection({ productId, seatMap, breakpoint } : SeatingSectionProps) {
  ...
  const renderSeat = (seatObj, index) => {
    ...
    if (type === "seat") {
      ...
      return (
        <Button key={`${id}-${index}`} className={`${className} border border-black text-black`}>
          {content}
        </Button>
      );
    } else if (type === "label") {
